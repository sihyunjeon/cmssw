...
std::vector<QCore> ReadoutChip::rem_duplicates(std::vector<QCore> qcores) {
    ...
    while (i < qcores.size()) {
        for (size_t j = i + 1; j < qcores.size();) {
            if (qcores[j].get_col() == qcores[i].get_col()
                && qcores[j].get_row() == qcores[i].get_row()) {
...
std::vector<QCore> ReadoutChip::organize_QCores(std::vector<QCore> qcores) {
    ...
    while (qcores.size() > 0) {
        ...
        for (size_t i = 1; i < qcores.size(); i++) {
            if (qcores[i].get_col() < qcores[min].get_col()) { min = i; }
            else if(qcores[i].get_col() == qcores[min].get_col() 
                    && qcores[i].get_row() < qcores[min].get_row()) { min = i; }
        }
