...
private:
  ...
  std::vector<Phase2TrackerCluster1D*> get_clusters_on_cic(edmNew::DetSetVector<Phase2TrackerCluster1D>::const_iterator clusterIterator, const TrackerGeometry& trackerGeometry, const int internal_id) 
  {
    ...
    if (sensor_unit == nullptr) throw cms::Exception("LogicError") << __FILE__ << " " << __LINE__ << "DetUnit not found";
...
public:
               ...
               const bool cic_id, const TrackerGeometry& trackerGeometry, const unsigned int eventId) : cic_id_(cic_id), eventId_(eventId)
  {
    ...
    if (sensor_1 != nullIter) sensor_1_clusters_ = get_clusters_on_cic(sensor_1, trackerGeometry, 1);
    if (sensor_2 != nullIter) sensor_2_clusters_ = get_clusters_on_cic(sensor_2, trackerGeometry, 2);
  }
