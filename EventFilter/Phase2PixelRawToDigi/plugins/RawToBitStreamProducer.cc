...
private:
  ...
  void processChip(
    const unsigned char* dataPtr, int chipStartWord, int chipEndWord, uint32_t detId,
    int chipId, edmNew::DetSetVector<Phase2ITChipBitStream>::FastFiller& filler);
...
void RawToBitstreamProducer::buildFedToModuleMapping() {
  ...
  for (const auto& pair : knownDTCIdsWithIndex) {
    ...
    for (int slinkId = 0; slinkId < SLINKS_PER_DTC; slinkId++) {
      ...
      if (modules_for_this_slink == 0) continue;
...
void RawToBitstreamProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (debug_) {
    std::cout << "\nProduced " << output->size() 
              << " DetSets of Phase2ITChipBitStream" << std::endl;
  }
  iEvent.put(std::move(output));
}

void RawToBitstreamProducer::processFED(const unsigned char* dataPtr, int fedSizeInWords, 
    ...
    for (int chipIdx = 0; chipIdx < numChips; chipIdx++) {
        ...
        if (moduleIdx >= numModules) moduleIdx = numModules - 1;
...
void RawToBitstreamProducer::processChip(
    const unsigned char* dataPtr, int chipStartWord, int chipEndWord, uint32_t detId,
    int chipId, edmNew::DetSetVector<Phase2ITChipBitStream>::FastFiller& filler) {
    ...
    if (debug) {
        std::cout << "  Header 1: " << wordToHexString(header1) << " (marker 0x" 
                  << std::hex << (header1 & 0xF000) << std::dec
    ...
    if (debug) {
        std::cout << "Added Phase2ITChipBitStream for chip " << chipId 
                  << " to filler" << std::endl;
    }
...
std::vector<uint32_t> RawToBitstreamProducer::extractChipOffsets(const unsigned char* dataPtr, int offsetStartWord, int maxWords) const {
...
std::vector<bool> RawToBitstreamProducer::extractBitstream(const unsigned char* dataPtr, int startWord, int bitstreamSize) const {
    ...
    if (debug_) std::cout << "UNPACKER: First 32 bits of extracted bitstream: " << getBitString(bitstream, 0, 32) << std::endl;
    return bitstream;
}
