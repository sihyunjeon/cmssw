...
options.register('outputEDM', 'aurora_output.root',
...
options.register('eventsPerStream', 16,
                 ...
                 'Aurora NE events per stream group, 1-64')
options.register('dropTot', 0,
                 VarParsing.VarParsing.multiplicity.singleton,
                 VarParsing.VarParsing.varType.int,
                 'Pack without the per-hit ToT')
...
if options.dropTot:
    process.PixelToBitStreamProducer.dropTot = cms.bool(True)
...
process.p = cms.Path(process.PixelToBitStreamProducer * process.BitStreamToAuroraProducer * process.auroraFilter)
